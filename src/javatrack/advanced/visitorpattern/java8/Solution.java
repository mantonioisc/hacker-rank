package javatrack.advanced.visitorpattern.java8;
...
public class Solution {
    ...
    private static void updateEdges(int from, int to) {
        Set<Integer> neighbors = edges.computeIfAbsent(from, ignore -> new HashSet<>());
        neighbors.add(to);
    }

    private static Tree addChildrenWithDFS(TreeNode parent, int parentIndex) {
        for (Integer childIndex : edges.get(parentIndex)) {
            Set<Integer> grandChildren = edges.getOrDefault(childIndex, Collections.emptySet()); //could be empty
